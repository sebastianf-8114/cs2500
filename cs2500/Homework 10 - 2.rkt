...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw10-problem2 (done)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; max-length : [BinTree-of Number] -> Number
; finds the length of the longest string in a binary tree of strings
(define (max-length x)
  (reduce-tree max string-length x))
...
; flatten : [BinTree-of Number] -> [List-of Num]
...
; map-tree : [BinTree-of Number] -> [BinTree-of Boolean]
...
(define (map-tree listoleaf func)
  (reduce-tree make-node (lambda (x) (make-leaf (func x))) listoleaf))
