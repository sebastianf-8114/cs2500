...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw12-problem1 done!+|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (count-strings los)
  (local
    [
     ; countHelper : [List-of String] [List-of SC] -> [List-of SC]
     ; adds supplied list of strings to current count
     (define (countHelper los losc)
       (cond
         [(empty? los) losc]
         [(cons? los)
          (countHelper (rest los) (accumulator (first los) losc))]))

     
     ; accumulator : String [List-of SC] -> [List-of SC]
     ; determines the count for the supplied string
     (define (accumulator string losc)
       (cond
         [(empty? losc) (list (make-sc string 1))]
         [(cons? losc)
          (if (correctstring string (first losc))
              (cons (newCount (first losc))
                    (rest losc))
              (cons (first losc)
                    (accumulator string (rest losc))))]))

     ; correctstring : String SC -> Boolean
     ; Checks to see if string matches string string count
     (define (correctstring string sc)
       (string=? (sc-str sc) string))

     ; newCount : StringCount -> StringCount
     ; add one to current StringCount
     (define (newCount numLosc)
       (make-sc (sc-str numLosc)
                (add1 (sc-count numLosc))))]
    (countHelper los '())))
