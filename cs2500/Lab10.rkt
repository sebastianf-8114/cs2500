...
(define (total-files dir)
  (+
   (foldr + 0 (map total-files (dir-dirs dir)))
...
(define (rename-files dir src dest)
  (local [; rename-files/dir : Directory -> Directory
          ; rename files in the supplied directory
          ...
          ; renames a file to dest if it's equal to src
...
(define (big-files dir min-size)
  (local [; file-big-enough? : File -> Boolean
          ...
          ; returns file names to keep based on size
          (define (files-to-keep lof)
            (map file-name
                 (filter file-big-enough? lof)))

          ; get-files : Direcory -> [List-of String]
          ; get all the files of a sufficient size
          (define (get-files d)
            (append
             (foldr append '() (map get-files (dir-dirs d)))
