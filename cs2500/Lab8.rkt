...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |lab8-starter done|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; ls->color-fn/improved : LetterStatus -> Image
; visualize a letter on a background associated with its status

(define (ls->color-fn/improved ls)
  (lambda (size) (square size "solid" (ls->color ls))))
...
; buffer : Image Image -> Image
; puts the supplued image (beside/above) the supplied background with a gap



(define (stack images func)
  (local [(define (buffer i bg)
            (func GAP i bg))]
