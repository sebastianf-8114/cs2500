...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw9-problem1 (almost+)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (read-dictionary x)
  (if
   (file-exists? x)
   (map string-upcase (read-lines x))
   '()))
...
(define (score 1string 2string)
  (local [ ; 1String 1String -> Pair
          ; takes in strings and appropriately assigns the correct letter status
              
          (define (pairMaker 1strings 11string)
            (cond
              [(string=? 1strings 11string) (make-pair 1strings LS-RIGHT)]
              [(and (string-in-list? 1strings (explode 2string))
                    (not
                     (string=? 11string 1strings))) (make-pair 1strings LS-MISPLACED)]
              [(not (string-in-list? 1strings (explode 2string))) (make-pair 1strings LS-WRONG)]))]
    (mymap2 (explode 1string) (explode 2string) pairMaker)))
...
;(define (partial-guess->full-guess s len)
; (string-append (implode (list (append (explode s) (- len (length (explode s))))) " "))


(define (partial-guess->full-guess s len)
  (implode (append (explode s) (build-list (- len (string-length s)) (lambda (space) " ")))))
...
(define (key-wordle wordState ke word num)
  (cond
    [(key=? "\r" ke) (returnHelp wordState word num)]
    [(key=? "\b" ke) (backspaceHelp wordState word num)]
    [(key-event? ke) (letterInput wordState ke num)]))

; returnHelp : WS [List-of String] Nat -> WS
; Checks to see if string is in the given list from return button
(define (returnHelp wordState word num)
  (if (string-in-list? (wordle-current wordState) word)
      (make-wordle (cons (wordle-current wordState) (wordle-past wordState)) "") wordState))

; backspaceHelp : WS -> WS
; eliminates end of a string
(define (backspaceHelp wordState word num)
  (if
   (string=? "" (wordle-current wordState))
   wordState
   (make-wordle (wordle-past wordState)
                (substring (wordle-current wordState)
                           0 (- (string-length
                                 (wordle-current wordState)) 1)))))
; letterInput : WS KeyEvent Nat -> WS
; adds a letter to the end of the WordState and makes a new one
(define (letterInput wordState ke num)
  (if
   (and
    (string-contains? ke "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
    (< (string-length (wordle-current wordState)) num))
   (make-wordle (wordle-past wordState) (string-append (wordle-current wordState) (string-upcase ke)))
   wordState))
...
(define (end-wordle? WS s num)
  (cond
    [(rightGuess? (wordle-past WS) s) #t]
    [(< num (length (wordle-past WS))) #t]
    [else #f]))


; rightGuess : WS String -> Boolean
; Determines if string is in wordState
(define (rightGuess? word s)
  (string-in-list? s word))
...
(define (draw-end-wordle wordleState 1string num)
  (cond
    [(ormap (lambda (input) (string=? input 1string)) (wordle-past wordleState))
     (overlay/align "right" "middle"
                    (text
                     (string-append "You WON! " "Total Number of Attempts: "
                                    (number->string (foldr guess 0 (wordle-past wordleState))))
                     16 "black") (empty-scene 300 300))]
    [(>= (foldr guess 0 (wordle-past wordleState)) num)
     (overlay/align "right" "middle"
                    (text
                     "YOU SUCK! GAME OVER!" 16 "black") (empty-scene 300 300))])) 
...
(define (draw-wordle wordleState 1string num)
  (stack/v
   (append
    (oldGuess wordleState 1string)
    (list (guess wordleState 1string))
    (newGuess wordleState 1string num))))

(define (oldGuess wordleState 1string)
  (map row->image (map (lambda (newlol) (map (lambda (lsp) (make-pair lsp lsp->image)) newlol))
                       (map (lambda (x) (score x 1string)) (wordle-past wordleState)))))

(define (guess wordleState 1string)
  (partial-guess->image (wordle-current wordleState) (string-length 1string)))

(define (newGuess wordleState 1string num)
  (build-list (- num (+ (length (wordle-past wordleState)) 1))
              (lambda (x) (partial-guess->image "" (string-length 1string)))))
